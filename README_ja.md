...
| `#scheduler_create_task`      | 確認後にスケジュールタスクを作成します。                                                                   |
| `#scheduler_update_task`      | 確認後にタスクの項目を更新します。有効/無効の変更は `#scheduler_set_task_enabled` を使います。             |
| `#scheduler_delete_task`      | タスク名・scope・ワークスペースを表示する強い確認後に削除します。                                          |
| `#scheduler_set_task_enabled` | 確認後にタスクを有効化または無効化します。                                                                 |

write 系ツールは既定で有効ですが、信頼済みワークスペースと Copilot Chat の確認が必要です。`copilotScheduler.lmTools.enableWriteTools` を `false` にすると、読み取り専用ツールだけを残して作成・更新・削除・有効/無効切替を無効化できます。
