...
| `copilotScheduler.jitterSeconds`          | `0`         | タスク実行前に入れるランダム遅延の最大秒数 (0=無効、タスクごとに上書き可) |
| `copilotScheduler.maxDailyExecutions`     | `50`        | 1日のスケジュール実行回数上限（全タスク合計、0=無制限）                   |
...
**免責:** この拡張機能は Copilot Chat を自動操作します。GitHub の Acceptable Use Policies は「過度な自動化された一括活動」や「スクリプトによる Copilot とのやり取り」を禁止しています。リスクを理解した上でご利用ください。ジッターや1日上限、長めの間隔はリスク低減になりますが、アカウント制限を防ぐ保証はありません。

※ 自動化ツールを使っていなくても Copilot アクセスが制限された事例があります。本拡張の緩和策はリスクを下げるだけで、リスクをゼロにはできません。
